...
def detect_outliers_zscore(df, column, threshold=3):
    """
    使用Z-score（标准分数）方法检测异常值

    Z-score方法原理：
    - Z = (x - μ) / σ，其中μ是均值，σ是标准差
    - 通常认为|Z| > 3的值为异常值（约占总数据的0.27%）
    - 基于正态分布假设，适用于近似正态分布的数据
    """
    # 计算Z分数：绝对值超过阈值（默认3）的点为异常值
...
def handle_outliers(df):
    ...
    处理流程：
    1. 识别所有数值型列
    2. 对每列使用IQR和Z-score两种方法检测异常值
    3. 生成异常值可视化图表
    4. 使用Winsorization方法处理异常值（用边界值替换）
    5. 生成处理总结报告
    """
    print("\n" + "=" * 60)
    print("步骤4: 异常值检测和处理")
    ...
    for col in numeric_cols:
        ...
        if outlier_count_iqr > 0:
            print(f"  异常值示例:")
            print(outliers_iqr[[col]].head(5))  # 显示前5个异常值示例

            # 创建双子图进行异常值可视化
            plt.figure(figsize=(12, 5))

            # 子图1: 箱线图（Box Plot）- 直观显示异常值
            plt.subplot(1, 2, 1)
            ...
            # 处理异常值：Winsorization（边界值替换法）
            # 将超出边界的异常值替换为边界值
            df_cleaned.loc[df_cleaned[col] < lower_bound, col] = lower_bound  # 下边界替换
            df_cleaned.loc[df_cleaned[col] > upper_bound, col] = upper_bound  # 上边界替换
            print(f"  已使用边界值替换异常值")
